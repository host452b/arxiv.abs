...
| 误解 | 为何盛行 | 实证结论 |
|---|---|---|
| 增加开发人员能加快延误的项目 | 直觉：更多人手 = 更快交付 | Brooks 定律（1975）：向延误项目增加人员只会使其更晚；沟通开销以 O(n²) 增长 |
| 从头重写能解决架构问题 | 全新开始的吸引力：干净的白板清除所有技术债 | 第二系统效应：重写会复现或放大原有设计缺陷；很少能按时按范围交付 |
| 好的代码不需要注释 | 整洁代码美学：自文档化代码是理想状态 | 注释解释的是"为什么"，而非"是什么"；研究表明无文档代码在维护期间的缺陷引入率更高 |

## 过时科学理论 / Obsolete Scientific Theories

| 理论 | 流行年代 | 被取代原因 |
|---|---|---|
| Jackson 结构化程序设计（JSP） | 1970s–1980s：数据结构驱动的程序设计 | 已被领域建模和 ORM 取代；对于事件驱动和分布式系统过于僵硬 |
| COCOMO I 成本估算模型 | 1981 年：代码行数作为主要成本驱动因素 | 被现代工具、IDE 和可复用库证伪；COCOMO II 和故事点模型更适合现代交付 |
| 瀑布模型作为标准软件开发生命周期 | 1970s–1990s：带阶段退出准则的顺序阶段 | 实证软件工程表明超过 60% 的需求在开发过程中发生变化；迭代方法已占主导 |

## 被证伪的理论 / Falsified Theories

| 理论 | 核心预测 | 证伪证据 |
|---|---|---|
| 软件复杂性随系统规模线性增长 | 预测：代码量翻倍 = 维护成本翻倍 | 实证研究支持 Lehman 定律：复杂性呈超线性增长；维护成本的增长速度快于规模 |
| 形式化验证消除所有 Bug | 预测：经数学证明的代码在运行时零故障 | Heartbleed（在经过验证审查的 OpenSSL 中）、Therac-25：验证范围遗漏了集成级和规格级的失败 |
