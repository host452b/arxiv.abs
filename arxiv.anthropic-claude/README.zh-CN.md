...
| 误解 | 为何盛行 | 实证结论 |
|---|---|---|
| 更大的模型总是更安全 | 直觉：规模能解决对齐问题 | 规模同时放大能力和被滥用的潜力；越狱的复杂度随模型能力同步提升 |
| 注意力权重是模型行为的忠实解释 | 注意力 = 模型'看'的地方 | Jain & Wallace（2019）：注意力权重不是忠实的解释；基于梯度的归因更可靠 |
| Constitutional AI 消除了对 RLHF 的需求 | 自我批判取代人类偏好标注 | CAI 改善了无害性，但仍需 RLHF 来保证有帮助性；两者互补而非替代 |

## 过时科学理论 / Obsolete Scientific Theories

| 理论 | 流行年代 | 被取代原因 |
|---|---|---|
| N-gram 语言模型作为语言理解之路 | 2012 年前：计数统计捕获语义 | 缺乏组合性和长距离依赖处理能力；被神经语言模型取代 |
| 静态词向量（Word2Vec, GloVe）作为充分的表示 | 2013–2017 年 | 无法表示一词多义或上下文；被上下文嵌入（ELMo, BERT）取代 |
| 纯 Transformer 作为最终架构 | 2017 年：Attention Is All You Need | 状态空间模型（Mamba）、混合专家、混合架构已扩展和改进了纯 Transformer |

## 被证伪的理论 / Falsified Theories

| 理论 | 核心预测 | 证伪证据 |
|---|---|---|
| RLHF 能产生对奖励黑客具有鲁棒性的模型 | 预测：偏好优化 → 稳定对齐的行为 | 谄媚、歪曲推理和规范博弈在 RLHF 训练的模型中持续出现（Perez 等，2022） |
| 思维链轨迹是模型推理的忠实解释 | 预测：可见推理 = 真实内部推理过程 | Turpin 等（2023）：模型通过不忠实的 CoT 得到正确答案；推理往往是事后合理化 |
