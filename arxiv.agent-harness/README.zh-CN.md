...
| 误解 | 为何盛行 | 实证结论 |
|---|---|---|
| 上下文窗口越大，Agent 性能越好 | 直觉：记忆越多 → 效果越好 | 注意力质量在接近上下文边界时下降；位置偏差导致中间内容盲区（Lost-in-the-Middle, 2023） |
| LLM Agent 已可投入生产自主运行 | 在精心设计的基准上取得高分 | SWE-bench：最佳 Agent 在无人类纠正的情况下，仅能完成不到 20% 的真实软件任务 |
| 工具箱中的工具越多，任务完成率越高 | 能力最大化主义 | 工具过载导致工具选择幻觉；3–5 个文档清晰的工具优于 20+ 个描述模糊的工具 |

## 过时科学理论 / Obsolete Scientific Theories

| 理论 | 流行年代 | 被取代原因 |
|---|---|---|
| 有限状态机（FSM）Agent | LLM 出现前，1980s–2010s | 面对开放式任务过于脆弱；已被 LLM + 工具调用架构取代 |
| GOFAI 规划 — STRIPS/PDDL | 1960s–2000s | 需要完整世界模型；在部分可观测场景下失败；已被神经规划器取代 |
| 多 Agent 协调的黑板架构 | 1970s–1980s | 隐式耦合导致协调失败；已被显式消息传递（A2A、函数调用）取代 |

## 被证伪的理论 / Falsified Theories

| 理论 | 核心预测 | 证伪证据 |
|---|---|---|
| 思维链长度与答案质量正相关 | 预测：推理步骤越长 → 准确率越高 | o1/o3 分析：超过最优长度后，扩展思考出现收益递减和错误放大 |
| 沙盒隔离能防止所有 Agent 副作用 | 预测：容器化执行 = 安全自主运行 | 通过工具输出进行的提示注入可绕过代码层隔离；Anthropic 红队研究（2024）已证实 |
